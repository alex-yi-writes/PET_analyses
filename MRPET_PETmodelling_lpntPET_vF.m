...
addpath(genpath('/Users/alex/Qumulo/IKND/Alex/paperwriting/MRPET/scripts/modelling'))
    ...
    for d = 1:2
        if days(i1,d) == 0
            ...
            if (i1==8 & d==1) | (i1==21 && d==1)
                TACs{i1,d} = load([paths.TACs_new num2str(IDs(i1)) num2str(d) '/' num2str(IDs(i1)) num2str(d) '_TACs_deccorr2_noPVC_newCerC.mat']);
            else
...
for id = 1:length(IDs)
    for d = 1:2
        if days(id,d) == 0      
            ...
            %%%%%%%%%%%%%%%
            PlotStrFit = 1; % plotting can be really annoying if you're trying to do something in parallel
            %%%%%%%%%%%%%%%
            ...
            for r=1
                ...
                for reg={'Caud','Put','Nac','ThalProp','Hipp','Amy','SN','LC','VTA'} % iterate through 10 ROIs (actually 9 because striatum is caud+put) we analyse now
                    ...
                    % APPROACH 1 (made model much noisier! revisit this later)
                    ...
                    %% ESRTM step-change search (still noisy, but better than lp-ntPET)

                    %%% APPROACH 1 (utter failure)
                    ...
                    % 
                    % TukeyBiweight = @(r) sum(1 - exp(-(r./5*mad(r,1)).^2)); % try to downweight outrageous residuals
                    % 
                    % best_rss = Inf;           % keep track of the best residual sum of squares
                    % BP_task = NaN;           % will become the final task-period BP
                    % best_bp{id,d} = NaN;           % remember which breakpoint was the winner
                    % BreakpointWindow = max(break_point,1) : min(break_point+2, t_points-1); % give it a jitter because the very first trial may not have been reward
                    % for newBreakPointWindow=BreakpointWindow
                    %     fESRTM    = @(bp_task) TukeyBiweight( ...
                    %         ROItac - simESRTMfixk2p_1_0_0( ...
                    %         tmidMin, reftac, t_points, R1_bl, k2p_esrtm, ...
                    %         [BP_base*ones(newBreakPointWindow-1,1); bp_task*ones(t_points-newBreakPointWindow+1,1)]) );
                    % 
                    %     [bp_task_tmp, rss_tmp] = fminbnd(@(x) fESRTM(x), 0, BP_base);
                    %     rss_tmp = fESRTM(bp_task_tmp)^2;   % rss = squared L2 norm
                    % 
                    %     % keep the winner 
                    %     if rss_tmp < best_rss
                    %         best_rss = rss_tmp;
                    %         BP_task  = bp_task_tmp;
                    %         best_bp{id,d}  = newBreakPointWindow;
                    %     end
                    % end
                    ...
                    % k2p_esrtm = k2_bl/R1_bl;                               % fix k2p (this is done to reduce free parameters from 4 to 3, giving the model enough stability for frame-by-frame fitting)
                    % 
                    % logitFun = @(x) 1./(1+exp(-x));                        % handy
                    % % params:  [BP_drop,   t_half,   slope]
                    % initP   = [ 0.4*BP_base,  tmidMin(break_point),  2 ];  % sensible seeds
                    % 
                    % objLogistic = @(p) norm( ROItac - ...
                    %     simESRTMfixk2p_1_0_0(tmidMin,reftac,t_points, ...
                    %     R1_bl,k2p_esrtm, ...
                    %     BP_base - p(1)*logitFun(p(3)*(tmidMin-p(2))) ));
                    % 
                    % pBest  = fminsearch(objLogistic, initP, optimset('MaxIter',2e4,'Display','off'));
                    % 
                    % BP_task    = BP_base - pBest(1);                 % value when A(t) saturated
                    % DeltaBP_ESRTM = pBest(1)/BP_base;                % same definition
                    % Occ_ESRTM     = 100*DeltaBP_ESRTM;


                    %%% APPROACH 3 (let's stick to what i've been doing)

                    fobj_bl = @(p) norm( ...
                        (simESRTM_1_0_0( ...
                        tmidMin(1:break_point-1), ...
                        reftac (1:break_point-1), ...
                        break_point-1,            ...
                        p(1), p(2), p(3)*ones(break_point-1,1)) ...
                        - ROItac(1:break_point-1)) ...
                        .* weighs(1:break_point-1) );

                    p0      = [1, 0.3, 2];                                  % [R1 k2 BP] seeds
                    optset  = optimset('Display','off','MaxIter',3e4,'MaxFunEvals',3e4);
                    p_bl    = fminsearch(fobj_bl, p0, optset);

                    R1_bl   = p_bl(1);
                    k2_bl   = p_bl(2);
                    BP_bl   = max(p_bl(3), 0);                              % clip at zero

                    stepFun = @(bp_task) norm( ...
                        (simESRTM_1_0_0( ...
                        tmidMin, reftac, t_points, ...
                        R1_bl, k2_bl, ...
                        [BP_bl*ones(break_point-1,1); ...
                        bp_task*ones(t_points-break_point+1,1)]) ...
                        - ROItac) ...
                        .* weighs );

                    BP_task = fminbnd(stepFun, 0, BP_bl);                  % 0 <= BP_task <= BP0

                    DeltaBP_ESRTM = BP_bl - BP_task;                       % absolute change
                    Occ_ESRTM     = 100 * DeltaBP_ESRTM / max(BP_bl, eps); % occupancy obviously

                    fprintf('ROI %-8s  BP0=%6.2f  BPtask=%6.2f  ΔBP_ESRTM=%6.2f  Occ=%6.1f%%\n', ...
                        reg{1}, BP_bl, BP_task, DeltaBP_ESRTM, Occ_ESRTM);
                    ...
                    % (but i'm really not sure if i understood everything correctly and carried them over to the code...)
                    % UPDATE: this was a total failure too. i sent the
                    % authour of b-ntPET paper an email to seek advice (may 2025)
                    
                    for FoldTheFailure=1
                    ...
                    % samples = zeros(nIter,7); % this will store every sampled theta vector for later
                    % 
...
save(['/Users/alex/Qumulo/IKND/Alex/paperwriting/MRPET/data/MRPET_BPpackage_condensed_' ...
...
outFile = ['/Users/alex/Qumulo/IKND/Alex/paperwriting/MRPET/data/MRPET_BPtable_condensedROIs_noPVC_' date '.xlsx'];
...
outFile = ['/Users/alex/Qumulo/IKND/Alex/paperwriting/MRPET/data/MRPET_BPbsl_condensedROIs_noPVC_' date '.xlsx'];
writetable(T,outFile,'WriteRowNames',true);
fprintf('excel sheet written:  %s\n',outFile);

%% save: ∆BP_overall only

ROIs = {'Amy','Hipp','LC','SN','ThalProp', ...
        'VTA'};
...
outFile = ['/Users/alex/Qumulo/IKND/Alex/paperwriting/MRPET/data/MRPET_BPchange_all_condensedROIs_noPVC_' date '.xlsx'];
writetable(T,outFile,'WriteRowNames',true);
fprintf('excel sheet written:  %s\n',outFile);
...
outFile = ['/Users/alex/Qumulo/IKND/Alex/paperwriting/MRPET/data/MRPET_BPchange_ESRTM_condensedROIs_noPVC_' date '.xlsx'];
writetable(T,outFile,'WriteRowNames',true);
fprintf('excel sheet written:  %s\n',outFile);
...
filename='MRPET_BPchange_ESRTM_condensedROIs_noPVC_14-Jun-2025';
...
% ======================================================== %
% bayesian ntPET (everything below here failed, don't use)
% ======================================================== %
